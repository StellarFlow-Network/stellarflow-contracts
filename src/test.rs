...
use crate::{
    ...
    TimeLockedUpgradeContractClient, DEFAULT_HEARTBEAT_INTERVAL, symbol_to_asset_id,
};
...
fn test_propose_upgrade() {
    ...
    assert_eq!(remaining.unwrap(), 5000u64);
}
...
fn test_execute_upgrade_after_timelock() {
    ...
    let remaining = client.get_upgrade_timelock_remaining();
    assert_eq!(remaining.unwrap(), 4999u64.saturating_sub(5001u64.saturating_sub(1)));
}
...
fn test_heartbeat_fresh_data() {
    ...
    // Use Symbol for get_last_update_timestamp (takes Symbol)
    let sym = symbol_short!("NGN");
    let asset: crate::AssetId = symbol_to_asset_id(&sym);

    client.update_heartbeat(&asset, &admin);

    assert!(client.is_data_fresh(&asset));

    let ts = client.get_last_update_timestamp(&crate::symbol_to_asset_id(&sym));
    assert!(ts.is_some());
...
fn test_heartbeat_never_updated() {
    ...
    let sym = symbol_short!("GHS");
    let asset: crate::AssetId = symbol_to_asset_id(&sym);

    assert!(!client.is_data_fresh(&asset));
    assert!(client.get_last_update_timestamp(&crate::symbol_to_asset_id(&sym)).is_none());
}
...
fn test_heartbeat_custom_interval() {
    ...
    advance_ledger_timestamp(&env, 300);
    assert!(!client.is_data_fresh(&asset));
}
...
fn test_is_data_fresh_does_not_mutate_state() {
    ...
    let asset: crate::AssetId = symbol_to_asset_id(&symbol_short!("NGN"));
...
fn test_query_methods_do_not_affect_each_other() {
    ...
    let asset: crate::AssetId = symbol_to_asset_id(&symbol_short!("KES"));
...
fn test_is_data_fresh_returns_false_for_unknown_asset() {
    ...
    let asset: crate::AssetId = symbol_to_asset_id(&symbol_short!("GHS"));
    assert!(!client.is_data_fresh(&asset));
}
...
fn test_stake_updates_heartbeat() {
    ...
    let stake_asset = crate::symbol_to_asset_id(&symbol_short!("STAKE"));
...
fn test_regional_feed_allows_lower_stake_than_premier_feed() {
    ...
    // Use Symbol for functions that expect Symbol
    let regional_sym = symbol_short!("KES");
    let premier_sym = symbol_short!("NGN");
    let signers = soroban_sdk::vec![&env, admin.clone(), admin.clone()];
    client.set_asset_feed_metrics(&admin, &regional_sym, &10, &100, &signers);
    client.set_asset_feed_metrics(&admin, &premier_sym, &80, &1_000, &signers);

    assert_eq!(client.get_staking_tier(&regional_sym), StakingTier::Regional);
    assert_eq!(client.get_staking_tier(&premier_sym), StakingTier::Premier);
    assert!(client.get_required_stake(&regional_sym) < client.get_required_stake(&premier_sym));

    let regional_record = client.stake_and_register_for_feed(&node, &regional_sym, &100u64);
    assert_eq!(regional_record.tier, StakingTier::Regional);
    assert_eq!(client.get_feed_stake(&node, &regional_sym), 100u64);

    let premier_result = client.try_stake_and_register_for_feed(&node, &premier_sym, &100u64);
    assert_eq!(
        premier_result,
        Err(Ok(ContractError::InsufficientStakeForTier))
    );

    let premier_record = client.stake_and_register_for_feed(&node, &premier_sym, &10_000u64);
    assert_eq!(premier_record.tier, StakingTier::Premier);
    assert_eq!(client.get_feed_stake(&node, &premier_sym), 10_000u64);
}
...
fn test_corridor_volume_bumps_tier_requirements() {
    ...
    let sym = symbol_short!("GHS");
    client.set_asset_feed_metrics(&admin, &sym, &10, &200, &soroban_sdk::vec![&env, admin.clone()]);

    assert_eq!(client.get_staking_tier(&sym), StakingTier::Regional);

    client.add_corridor_fees(&sym, &2_000_000_000u64, &0u64);

    assert_eq!(client.get_staking_tier(&sym), StakingTier::Standard);
    assert_eq!(client.get_required_stake(&sym), 1_000u64);
}
...
fn test_custom_tier_config_is_enforced() {
    ...
    let sym = symbol_short!("ZAR");
    client.set_asset_feed_metrics(&admin, &sym, &10, &100, &signers);

    assert_eq!(client.get_required_stake(&sym), 250u64);

    let result = client.try_stake_and_register_for_feed(&node, &sym, &200u64);
    assert_eq!(result, Err(Ok(ContractError::InsufficientStakeForTier)));

    client.stake_and_register_for_feed(&node, &sym, &250u64);
    assert_eq!(client.get_feed_stake(&node, &sym), 250u64);
}
...
fn test_unstake_from_feed_updates_totals() {
    ...
    let sym = symbol_short!("UGX");
    client.set_asset_feed_metrics(&admin, &sym, &10, &100, &soroban_sdk::vec![&env, admin.clone()]);
    client.stake_and_register_for_feed(&node, &sym, &100u64);

    assert_eq!(client.get_total_staked(), 100u64);
    assert_eq!(client.unstake_from_feed(&node, &sym), 100u64);
    assert_eq!(client.get_feed_stake(&node, &sym), 0u64);
    assert_eq!(client.get_total_staked(), 0u64);
}
...
fn test_set_value_updates_heartbeat() {
    ...
    let value_asset = crate::symbol_to_asset_id(&symbol_short!("VALUE"));
    ...
    assert!(client.is_data_fresh(&value_asset));
    assert!(client.get_last_update_timestamp(&crate::symbol_to_asset_id(&symbol_short!("VALUE"))).is_some());
...
fn test_emergency_revocation_proposal_opens_successfully() {
    ...
    let proposal = client.get_revocation_proposal();
